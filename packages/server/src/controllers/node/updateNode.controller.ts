...
export const updateNodeController =
  ...
  async (req: Request<UpdateNodeParams, {}, Partial<Omit<Node, 'id'>>>, res: TypedResponse): Promise<TypedResponse> => {
    ...
    if (!(await getNode(id)))
      return res.status(StatusCode.NOT_FOUND).json({ message: 'Node not found', success: false })
