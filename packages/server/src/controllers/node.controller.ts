...
const { getNodes, getNode, createNode, updateNode, deleteNode, bulkCreate, bulkUpdate, bulkDelete } = nodeService

export const nodeController = {
  getNodes: async (_req: Request, res: TypedResponse<SnakeCase<Node>[]>): Promise<TypedResponse<SnakeCase<Node>[]>> => {
  ...
  getNode: async (req: Request<{ id: string }>, res: TypedResponse): Promise<TypedResponse<Node>> => {
    const { id } = req.params
  ...
  createNode: async (req: Request<{}, {}, SnakeCase<CreateNodeBody>>, res: TypedResponse<SnakeCase<Node>>): Promise<TypedResponse<SnakeCase<Node>>> => {
  ...
  updateNode: async (req: Request<UpdateNodeParams, {}, Partial<Omit<Node, 'id'>>>, res: TypedResponse): Promise<TypedResponse> => {
    const { id } = req.params
  ...
  deleteNode: async (req: Request<{ id: string }>, res: TypedResponse<{ message: string }>): Promise<TypedResponse<{ message: string }>> => {
    const { id } = req.params
    if (!id) return res.status(StatusCode.BAD_REQUEST).json({ message: 'Bad Request: Missing required fields', success: false })
    if (!await getNode(id)) return res.status(StatusCode.NOT_FOUND).json({ message: 'Node not found', success: false })
  ...
  bulkCreate: async (req: Request<{}, {}, SnakeCase<CreateNodeBody>[]>, res: TypedResponse): Promise<TypedResponse> => {
    ...
    const resultLength = await bulkCreate(nodesToInsert)

    return res.status(StatusCode.CREATED).json({ message: `${resultLength} Nodes created successfully`, success: true })
  },
  bulkUpdate: async (req: Request<{}, {}, SnakeCase<Partial<UpdateNodeBody>>[]>, res: TypedResponse): Promise<TypedResponse> => {
    ...
    const resultLength = await bulkUpdate(updatePayloads)

    return res.status(StatusCode.OK).json({ message: `${resultLength} Nodes updated successfully`, success: true })
  },
  bulkDelete: async (req: Request<{}, {}, SnakeCase<DeleteNodeBody>[]>, res: TypedResponse): Promise<TypedResponse> => {
    ...
    const resultLength = await bulkDelete(idsToDelete)
