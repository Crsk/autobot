...
import { addNode, updateConnections, updateNodeCoords } from '@/redux/slices/nodeSlice'
...
const useDrag = (elementRef: any, nodeId: string, newNode: { parentId: string, ref: any } | undefined = undefined) => {
  ...
  useEffect(() => {
    const updateNodeAndConnections = (x: number, y: number, snap: boolean = false) => {
      ...
      else dispatch(updateNodeCoords({ id: nodeId, x: snap ? snapToGrid(x) : x, y: snap ? snapToGrid(y) : y }))
