...
const queueEpic: Epic<any, any, RootState> = (action$, state$) => action$.pipe(
  ...
  mergeMap(() => {
    const { ADD, UPDATE, DELETE }: QueueOperation<AddNodePayload, UpdateNodePayload, DeleteNodePayload> = state$.value.queue.NODE
    const unsyncedNodes: { actionType: QueueActionTypes, payload: (AddNodePayload | UpdateNodePayload | DeleteNodePayload) }[] = [
